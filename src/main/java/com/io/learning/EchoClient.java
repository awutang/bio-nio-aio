...
public class EchoClient {
    ...
    public void start() throws Exception {
        ...
        try {
            Bootstrap b = new Bootstrap();                //1 一个 Bootstrap 被创建来初始化客户端
            b.group(group)                                //2 指定 EventLoopGroup 来处理客户端事件。由于我们使用 NIO 传输，所以用到了 NioEventLoopGroup 的实现
                    .channel(NioSocketChannel.class)            //3 使用的 channel 类型是一个用于 NIO 传输的
            ...
            group.shutdownGracefully().sync();            //8 关闭线程池和释放所有资源
        }
    }
